...
export default function SimpleSwapDemo() {
  ...
  async function connectWallet() {
    if (!window.ethereum) return alert("No Web3 wallet found. Install MetaMask.");
    try {
      const p = new ethers.providers.Web3Provider(window.ethereum, "any");
  ...
  async function approveIfNeeded(onChainIn) {
    ...
    try {
      ...
      await tx.wait();
      const newAllowance = await token.allowance(await signer.getAddress(), SIMPLE_SWAP_ADDRESS);
  ...
  return (
    <div className="max-w-md mx-auto mt-8 p-6 rounded-2xl shadow-lg bg-gradient-to-br from-[#0b0720] via-[#1a0b2a] to-[#2b0f36] text-white">
      <h2 className="text-2xl font-bold text-center">Demo Swap (Polygon Amoy Testnet)</h2>
      <p className="text-sm text-center mt-1">Swap IDRX ↔ USDC — UI sederhana, rate 1:1</p>

      <div className="mt-4 text-xs text-gray-300">Connected: {account ? account : "Not connected"}</div>
      ...
      <div className="mt-4">
        ...
        <div className="flex gap-3 items-center">
          <input
            ...
            inputMode="decimal"
          />
          <div className="px-3 py-2 border border-white/20 rounded">{direction === "IDRXtoUSDC" ? "IDRX" : "USDC"}</div>
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="flex gap-3 items-center">
          <div className="flex-1 bg-transparent border border-white/30 rounded px-3 py-2">{estimatedOut}</div>
          <div className="px-3 py-2 border border-white/20 rounded">{direction === "IDRXtoUSDC" ? "USDC" : "IDRX"}</div>
        </div>
      </div>
      ...
      <div className="mt-4 flex gap-2">
        <button
          ...
          onClick={() => setDirection(direction === "IDRXtoUSDC" ? "USDCtoIDRX" : "IDRXtoUSDC")}
        >
          Switch Direction
        </button>
        <button
      ...
      <div className="mt-6">
        <button
          className={`w-full py-3 rounded-xl font-bold ${isSwapping || isApproving ? "bg-gray-500" : "bg-pink-600 hover:brightness-110"}`}
          onClick={doSwap}
      ...
      {txHash && (
        <div className="mt-3 text-sm text-gray-200 break-all">Last tx: {txHash}</div>
      )}

      <div className="mt-4 text-xs text-gray-400">
        <div>Network: {network ? `${network.name} (${network.chainId})` : "-"}</div>
        <div className="mt-2">UI paksa jaringan Polygon Amoy Testnet (80002). Jika MetaMask ada di jaringan lain, otomatis akan diminta pindah.</div>
      </div>
    </div>
  );
}
